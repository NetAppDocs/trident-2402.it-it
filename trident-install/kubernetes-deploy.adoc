...
keywords: deploy, astra trident, methods, operator, tridentctl, helm, tanzu, vmware, netapp console 
...
* Privilegi completi per un cluster Kubernetes supportato che esegue una versione supportata di Kubernetes e con i requisiti delle funzionalità abilitati. Consultare link:../trident-get-started/requirements.html["requisiti"] per i dettagli.
...
Seleziona il metodo di installazione più adatto alle tue esigenze. Dovresti anche esaminare le considerazioni per link:kubernetes-deploy.html#moving-between-installation-methods["passaggio da un metodo all'altro"] prima di prendere una decisione.
...
Che tu faccia il deployment manualmente o usando Helm, l'operatore Trident è un ottimo modo per semplificare l'installazione e gestire dinamicamente le risorse Astra Trident. Puoi persino link:kubernetes-customize-deploy.html["Personalizzare l'implementazione dell'operatore Trident"] usando gli attributi nella  `TridentOrchestrator` custom resource (CR).
...
====
.<strong>Gestione del cluster Kubernetes tramite NetApp Console</strong>
[%collapsible]
====
Con Astra Trident tramite NetApp Console, puoi eseguire l'aggiornamento all'ultima versione di Astra Trident, aggiungere e gestire classi di archiviazione e connetterle agli ambienti di lavoro, nonché eseguire il backup di volumi persistenti utilizzando Cloud Backup Service.  La console supporta la distribuzione Astra Trident tramite l'operatore Trident , manualmente o tramite Helm.
