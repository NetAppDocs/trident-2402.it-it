...
* Un problema comune che potresti riscontrare quando utilizzi Trident https://azure.microsoft.com/en-us/products/netapp/["Azure NetApp Files"] è quando i tenant e i segreti del client provengono da una registrazione dell'app con autorizzazioni insufficienti. Per un elenco completo dei requisiti di Trident, consulta la link:trident-use/anf.html["Azure NetApp Files"] configurazione.
...
Quando si utilizza nvme/tcp in ambienti k8s con nomi di nodi lunghi, i pod non possono collegarsi allo storage.

.Soluzione alternativa
Accorciare il nome del nodo in modo che il nome del sottosistema sia inferiore a 64 caratteri.
