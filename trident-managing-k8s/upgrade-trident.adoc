...
In genere, dovresti eseguire l'aggiornamento utilizzando lo stesso metodo che hai usato per l'installazione iniziale, tuttavia puoi link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["passare da un metodo di installazione all'altro"]. Ci sono due opzioni per aggiornare Astra Trident.
