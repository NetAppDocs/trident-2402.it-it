...
[role="lead"]
Puoi aggiornare Astra Trident usando l'operatore Trident, sia manualmente che tramite Helm. Puoi eseguire l'aggiornamento da un'installazione dell'operatore Trident a un'altra installazione dell'operatore Trident oppure aggiornare da un'installazione  `tridentctl` a una versione dell'operatore Trident. Consulta link:upgrade-trident.html#step-3-select-an-upgrade-method["Selezionare un metodo di aggiornamento"] prima di aggiornare un'installazione dell'operatore Trident.
...
NOTE: Per eseguire l'aggiornamento da Astra Trident installato utilizzando l'operatore namespace-scoped (versioni 20.07 attraverso 20.10), utilizzare le istruzioni di aggiornamento per la versione di Astra Trident installata.
...
.Fasi
. Se link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["Installato Astra Trident utilizzando Helm"], puoi usare  `helm upgrade trident netapp-trident/trident-operator --version 100.2402.0` per aggiornare in un unico passaggio. Se non hai aggiunto il Helm repo o non puoi usarlo per aggiornare:
+
...
NOTE: Prima di passare da un metodo di installazione all'altro, rivedere link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["Passaggio da un metodo di installazione all'altro"].
