...
.Fasi
. Se link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["Installato Astra Trident utilizzando Helm"], puoi usare  `helm upgrade trident netapp-trident/trident-operator --version 100.2402.0` per aggiornare in un unico passaggio. Se non hai aggiunto il Helm repo o non puoi usarlo per aggiornare:
+
