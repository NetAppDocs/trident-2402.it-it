...
Uno dei tanti link:../trident-install/kubernetes-deploy.html["Vantaggi dell'utilizzo dell'operatore Trident"] modi per installare e aggiornare Astra Trident è la gestione automatica degli oggetti Astra Trident e Kubernetes senza interrompere i volumi montati esistenti. In questo modo, Astra Trident può supportare gli aggiornamenti senza tempi di inattività, ovvero link:https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/["_rolling updates_"^]. In particolare, l'operatore Trident comunica con il cluster Kubernetes per:
...
TIP: Per ulteriori informazioni sull'architettura di Astra Trident nel cluster Kubernetes, consulta link:../trident-get-started/architecture.html["Architettura Astra Trident"].
