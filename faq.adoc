...
Sì, Astra Trident può essere installato offline. Consulta link:./trident-install/kubernetes-deploy.html["Scopri di più sull'installazione di Astra Trident"^].
...
Il programma di installazione di Trident crea un Kubernetes ClusterRole, che ha accesso specifico alle risorse PersistentVolume, PersistentVolumeClaim, StorageClass e Secret del cluster Kubernetes. Consulta link:./trident-install/kubernetes-customize-deploy-tridentctl.html["Personalizzare l'installazione di tridentctl"^].
...
Puoi generare e modificare localmente i file manifest esatti che Astra Trident utilizza per l'installazione, se necessario. Consulta link:./trident-install/kubernetes-customize-deploy-tridentctl.html["Personalizzare l'installazione di tridentctl"^].
