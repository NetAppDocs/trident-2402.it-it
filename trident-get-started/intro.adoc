...
Per effettuare una prova, richiedi l'accesso a "Easily Deploy and Clone Persistent Storage for Containerized Workloads" link:https://www.netapp.com/test-drives/["Test drive di NetApp"^] utilizzando un'immagine di laboratorio pronta all'uso. La prova offre un ambiente sandbox con un cluster Kubernetes a tre nodi e Astra Trident installato e configurato. Questo è un ottimo modo per familiarizzare con Astra Trident ed esplorarne le funzionalità.
